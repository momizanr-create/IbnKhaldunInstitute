...
async function buildInitialBundle() {
  ...
  const [
    ...
    safe(Settings.findOne({ key: 'site_settings' }).then(s => s && s.value || null)),
    safe(Settings.findOne({ key: 'theme_settings' }).then(s => s && s.value || null)),
    safe(Settings.findOne({ key: 'hero_section' }).then(s => s && s.value || null)),
    safe(Settings.findOne({ key: 'navigation' }).then(s => s && s.value || null)),
    safe(Settings.findOne({ key: 'footer' }).then(s => s && s.value || null)),
    safe(Settings.findOne({ key: 'welcome_popup' }).then(s => s && s.value || null)),
    safe(Settings.findOne({ key: 'cta_section' }).then(s => s && s.value || null)),
    safe(Settings.findOne({ key: 'featured_courses_config' }).then(s => s && s.value || null)),
    safe(Course.find({}).lean()),
    safe(Testimonial.find({}).lean()),
    safe(Instructor.find({}).lean()),
    safe(Notice.find({}).lean()),
    safe(BlogPost.find({}).lean()),
    safe(Settings.findOne({ key: 'contact_content' }).then(s => s && s.value || null)),
