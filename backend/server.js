...
app.get('/api/courses', async (req, res) => {
  try {
    ...
    // SEARCH FIX #4: সার্চ ফিল্টার যোগ করা
    if (req.query.search) {
...
// FIX #3: একটি নির্দিষ্ট কোর্সের নতুন মন্তব্য ও রিপ্লাই সিঙ্ক করা (polling এর জন্য)
app.get('/api/courses/:id/comments/updated/:since', async (req, res) => {
  try {
    const since = new Date(Number(req.params.since) || 0);
    const list = await CourseComment.find({
      courseId: req.params.id,
      $or: [
        { createdAt: { $gte: since } },
        { updatedAt: { $gte: since } },
        { adminReplyAt: { $gte: since } }
      ]
    }).sort({ createdAt: -1 }).limit(100);
    res.json(list);
  } catch (e) { res.status(500).json({ error: e.message }); }
